...
use Huixiaoer\Zipkin\Manager\TracerManager;
...
class CoroutineAdapter implements AdapterInterface
{
    ...
    public function request(RequestInterface $request, array $options = []): HttpResultInterface
    {
        ...
        if (null !== $client->errCode && $client->errCode !== 0) {
            App::trigger('HttpClient', 'error',sprintf('HttpClient Request ERROR #%s url=%s, error message=%s', $client->errCode, $url,\socket_strerror($client->errCode)));
